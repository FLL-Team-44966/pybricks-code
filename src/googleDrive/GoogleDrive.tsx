...
export default function DownloadPicker() {
    ...
    const openDownloadPicker = () => {
        ...
        // Use native Google Picker API for better navigation and multiple views
        ...
        if (authToken) {
            // Use native picker with multiple views (My Drive, Recent, Shared with me)
            openNativeGooglePicker(handlePickerCallback);
        } else {
            // Fallback to the wrapper if no auth token (will trigger auth flow)
